    ...
    <Card className="@container/card flex min-h-0 flex-1 flex-col">
        ...
        {error ? (
          <Alert variant="destructive">
            ...
            <AlertDescription className="flex flex-wrap items-center justify-between gap-2">
              <span>{t("watchdog.activity_load_error")}</span>
              <Button
                type="button"
                variant="outline"
                size="sm"
              ...
              <TableHeader>
                <TableRow className="bg-muted/30 hover:bg-muted/30">
                  <TableHead className="hidden @md/card:table-cell">
                    {t("watchdog.activity_col_event")}
                  </TableHead>
                  <TableHead>{t("watchdog.activity_col_detail")}</TableHead>
                  <TableHead className="text-right @md/card:text-left">
                    {t("watchdog.activity_col_time")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading && recoveryEvents.length === 0 ? (
                    ...
                    <TableCell colSpan={3} className="py-12">
                      <div className="flex flex-col items-center gap-2 text-center">
                        ...
                        <p className="text-sm font-medium">
                          {t("watchdog.activity_empty_title")}
                        </p>
                        <p className="text-muted-foreground max-w-sm text-xs">
                          {t("watchdog.activity_empty_message")}
                        </p>
                      </div>
                    </TableCell>
                  </TableRow>
                  ...
                  pageEvents.map((event, index) => {
                    ...
                    const label = tEvents(`type.${event.type}`, {
      ...
      {!error && recoveryEvents.length > PAGE_SIZE && (
        <CardFooter className="justify-between gap-3 border-t pt-4">
          <span className="text-muted-foreground text-xs tabular-nums">
            {t("watchdog.activity_page_status", {
              page: safePage + 1,
              total: pageCount,
            })}
          </span>
